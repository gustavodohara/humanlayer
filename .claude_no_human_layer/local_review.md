...
You are tasked with setting up a local review environment for a colleague's branch. This involves creating a worktree, setting up dependencies, and opening your editor.
...
4. **Configure the worktree**:
   - (Optional) Copy editor settings: `cp .editorconfig WORKTREE/ 2>/dev/null || true`
   - Run setup: `make -C WORKTREE setup` (or your project-specific setup command)
   - (Optional) Initialize docs folder for collaboration: `mkdir -p WORKTREE/docs`
